...
CREATE TABLE native_jdbc_study.employee (
	...
	dno     INT(11)     NULL     DEFAULT 1 COMMENT '부서번호', -- 부서번호
...
-- 사원
ALTER TABLE native_jdbc_study.employee
	ADD CONSTRAINT fk_employee_dno -- fk_employee_dno
		FOREIGN KEY (
			dno -- 부서번호
		)
	...
	ADD INDEX fk_employee_dno (
		dno -- 부서번호
	);

-- 사원
ALTER TABLE native_jdbc_study.employee
	ADD CONSTRAINT fk_employee_manager -- fk_employee_manager
		FOREIGN KEY (
			manager -- 직속상사
		)
